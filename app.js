...
// Los freezers que ya estaban entregados antes de la 8.0 quedaron sin la carga
// inicial registrada, asi que la regla de los 45 dias los contaba desde su
// fecha de ingreso (meses atras) y los mandaba a prospecto una semana despues
// de haberles instalado el equipo. Se les pone la fecha de entrega como primer
// pedido, que es lo que realmente paso.
function migrarCargaInicial(){
  if(!D.user||D.user.r!=='admin')return;
  if(!CFG_CARGADA)return;
  if(D.cfg&&D.cfg.cargaInicialV>=1)return;
  var arreglados=[];
  (D.com||[]).forEach(function(co){
    if(estadoComodato(co)!=='activo'||!co.fe)return;
    var c=D.cli.find(function(x){return x.id===co.cid;});
    if(!c||c.eliminado)return;
    if(c.uv)return;                       // ya tiene un pedido registrado: no se toca
    c.uv=co.fe;
    c.esP=false;
    if(!c.etapaEmbudo||c.etapaEmbudo!=='Cliente Activo')c.etapaEmbudo='Cliente Activo';
    c._modBy='Sistema';c._modAt=new Date().toISOString();
    fsSetContacto(c);
    co.cargaInicial=true;fsSetComodato(co);
    arreglados.push(c.nm);
    logEvento('venta',c.id,c.nm,'Carga inicial registrada como primer pedido (freezer entregado el '+fmt(co.fe)+')','','');
  });
  D.cfg.cargaInicialV=1;
  fsSetConfig({cargaInicialV:1});
  if(arreglados.length)toast(arreglados.length+' cliente'+(arreglados.length!==1?'s':'')+' con freezer: se registro su carga inicial','ok');
}
// Limpia de la base la marca vieja "deudor si/no". Quedaron contactos
// marcados sin ningun movimiento cargado: figuraban como deudores y en Ventas
// no les aparecia nada. El saldo real es el unico dato valido.
function limpiarMarcaDeudorVieja(){
  if(!D.user||D.user.r!=='admin')return;
  if(!CFG_CARGADA)return;
  if(D.cfg&&D.cfg.deuLimpiadaV>=1)return;
  var conMarca=D.cli.filter(function(c){return c.deu!==undefined;});
  conMarca.forEach(function(c){delete c.deu;fsSetContacto(c);});
...
// Freezers a retirar: entregados, en un contacto que hace mas dias que el
// plazo configurado que no registra un pedido. Usa el mismo plazo que la regla
// de recaida, porque es la misma decision comercial.
function freezersARetirar(){
  var lim=diasRecaida();
  var out=[];
  (D.com||[]).forEach(function(co){
    if(estadoComodato(co)!=='activo')return;
    var c=D.cli.find(function(x){return x.id===co.cid;});
    if(!c||c.eliminado)return;
    if(D.user&&D.user.r==='vendedor'&&c.vend!==D.user.n)return;
    var ref=c.uv||co.fe||c.ing;      // ultimo pedido; si nunca hubo, desde que se entrego
    if(!ref)return;
    var d=dias(ref);
    if(d===null||d<lim)return;
    out.push({c:c,co:co,dias:d,desde:ref,nunca:!c.uv});
  });
  out.sort(function(a,b){return b.dias-a.dias;});
  return out;
}
function retirarDesdePanel(coid){
  retCo(coid);
  verRetiros();
}
function verRetiros(){
  var lista=freezersARetirar();
  var lim=diasRecaida();
  var h='';
  if(!lista.length){h='<div class="empty">Ningun freezer para retirar. Todos los equipos estan trabajando.</div>';}
  else{
    h='<div style="font-size:12px;color:var(--muted);margin-bottom:10px">Tienen nuestro freezer y hace mas de '+lim+' dias que no pasan un pedido. Agrupados por barrio, del que mas tiempo lleva al que menos.</div>';
    agruparPorBarrio(lista).forEach(function(g){
      h+=encabezadoBarrio(g);
      g.items.forEach(function(it){
        var c=it.c;
        h+='<div class="cc" style="margin-bottom:8px;border-left:4px solid var(--red)">';
        h+='<div style="margin-bottom:8px"><div style="font-size:14px;font-weight:700">'+es(c.nm)+'</div>';
        if(c.dir)h+='<div style="font-size:11px;color:var(--muted)">&#128205; '+es(c.dir)+'</div>';
        h+='<div style="font-size:11px;color:var(--red);font-weight:700">Freezer '+(it.co.nro?'#'+es(it.co.nro):'sin N°')+(it.co.marca?' · '+es(it.co.marca):'')+'</div>';
        h+='<div style="font-size:11px;color:var(--muted)">'+(it.nunca?'Nunca registro un pedido · entregado el '+fmt(it.desde):'Ultimo pedido: '+fmt(it.desde))+' · hace '+it.dias+' dias</div>';
        var salR=saldoDe(c.id);
        if(salR>0)h+='<div style="font-size:11px;color:var(--red);font-weight:800;margin-top:2px">&#9888; Ademas debe '+plata(salR)+'</div>';
        h+='</div>';
        h+='<div style="display:flex;gap:6px;flex-wrap:wrap">';
        h+='<button class="sm g" onclick="agregarAGiraRapido(\''+c.id+'\',\'ret\')">+ Gira para retirar</button>';
        h+='<button class="sm" onclick="cMod();abrirVisita(\''+c.id+'\')">Visitar</button>';
        if(c.tel)h+='<button class="sm wa" onclick="envWA(\''+c.id+'\')">WhatsApp</button>';
        h+='<button class="sm" onclick="retirarDesdePanel(\''+it.co.id+'\')" style="color:var(--orange)">Ya lo retire</button>';
        h+='</div></div>';
      });
    });
  }
  oMod('Freezers a retirar ('+lista.length+')',h);
}
...
// Fecha en que se le entrego el freezer que hoy tiene puesto (el mas reciente).
function fechaFreezerPuesto(cid){
  var fs=(D.com||[]).filter(function(co){return co.cid===cid&&estadoComodato(co)==='activo'&&co.fe;})
    .map(function(co){return co.fe;}).sort();
  return fs.length?fs[fs.length-1]:'';
}
// Desde cuando se cuenta la inactividad de un cliente:
//   1) su ultimo pedido;
//   2) si nunca compro, la fecha en que se le puso el freezer (poner un equipo
//      es empezar de cero: no se puede castigar por lo que paso antes);
//   3) si tampoco hay freezer, la fecha de ingreso.
function refInactividad(c){
  return c.uv||fechaFreezerPuesto(c.id)||c.ing;
}
function clientesEnRecaida(){
  var lim=diasRecaida();
  return D.cli.filter(function(c){
    if(c.esP||c.eliminado)return false;
    var ref=refInactividad(c);
...
function revisarClientesInactivos(){
  ...
  lista.forEach(function(c){
    var ref=refInactividad(c);
    var fz=fechaFreezerPuesto(c.id);
    var motivo=c.uv?('sin pedidos desde '+fmt(c.uv)):(fz?('nunca registro un pedido, freezer puesto el '+fmt(fz)):('nunca registro un pedido, ingreso '+fmt(c.ing)));
    c.esP=true;c.etapaEmbudo='Negociacion';
    c._modBy='Sistema';c._modAt=new Date().toISOString();
    fsSetContacto(c);
...
function normalizarDatos(){
  ...
  D.cli.forEach(function(c){if(!c.etapaEmbudo||c.etapaEmbudo==='')c.etapaEmbudo=c.esP?'Nuevo Prospecto':'Cliente Activo';if(!c.uv)c.uv='';if(!c.ex)c.ex={};if(c.deu!==undefined)delete c.deu;if(!c.ing)c.ing='';if(!c.vend)c.vend='';if(!c.prods)c.prods=[];if(!c.prov)c.prov='';if(!c.ciu)c.ciu='';if(c.agendado===undefined)c.agendado=false;if(!c.tel2)c.tel2='';if(!c.email)c.email='';if(c.eliminado===undefined)c.eliminado=false;if(!c.fuente)c.fuente=c.esP?'Prospeccion directa':'';});
...
function renderVH(){
  ...
  // Freezers que hay que ir a buscar
  var retL=freezersARetirar();
  if(retL.length){
    h+='<div style="background:rgba(248,113,113,.10);border:1px solid rgba(248,113,113,.35);border-radius:var(--rsm);padding:10px 12px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="verRetiros()">';
    h+='<div><div style="font-size:13px;font-weight:700;color:var(--red)">&#10052; '+retL.length+' freezer'+(retL.length!==1?'s':'')+' para retirar</div>';
    h+='<div style="font-size:11px;color:var(--muted);margin-top:2px">Sin pedidos hace mas de '+diasRecaida()+' dias · el mas viejo hace '+retL[0].dias+'</div></div>';
    h+='<span style="color:var(--red);font-size:20px">&rsaquo;</span></div>';
  }
  ...
  if(deuL.length){
    ...
    var rotas=deuL.filter(function(d){return d.promVenc;}).length;
    var hoyProm=deuL.filter(function(d){return d.promHoy;}).length;
    ...
    var sub=[];
    if(hoyProm)sub.push(hoyProm+' viene'+(hoyProm!==1?'n':'')+' a pagar HOY');
    if(rotas)sub.push(rotas+' rompio'+(rotas!==1?'n':'')+' la promesa');
    if(!sub.length&&viejo)sub.push('el mas viejo hace '+viejo+' dias');
    h+='<div style="font-size:11px;color:'+(rotas?'var(--red)':'var(--muted)')+';margin-top:2px">'+(sub.join(' · ')||'Agrupados por barrio')+'</div></div>';
    h+='<span style="color:var(--red);font-size:20px">&rsaquo;</span></div>';
  }
...
function agregarAGiraRapido(cid,volverA){
  ...
  if(volverA==='seg')verSeguimiento();else if(volverA==='deu')verDeudores();else if(volverA==='ret')verRetiros();else verInaccion();
}
...
function setFiltro(k,v){
  ...
  document.querySelectorAll('[data-fk="'+k+'"]').forEach(function(el){
    el.classList.toggle('on',vcF[k].indexOf(el.getAttribute('data-fv'))>=0);
  });
}
function abrirFiltros(){
  ...
  // Cada filtro es un grupo de chips: se pueden marcar varios de cada uno.
  // Varios marcados = "de este O de este". Sin marcar ninguno = todos.
  function fgrupo(k,lbl,opciones){
    var h='<div class="fg"><div class="fl">'+lbl+'</div><div class="chips">';
    opciones.forEach(function(o){
      var v=Array.isArray(o)?o[0]:o;var l=Array.isArray(o)?o[1]:o;
      var active=vcF[k].indexOf(v)>=0;
      h+='<span class="ch'+(active?' on':'')+'" data-fk="'+k+'" data-fv="'+es(v)+'" onclick="setFiltro(this.dataset.fk,this.dataset.fv)" style="font-size:12px;padding:6px 11px">'+es(l)+'</span>';
    });
    h+='</div></div>';
    return h;
  }
  ...
  if(tipos.length)h+=fgrupo('tipOneg','Tipo de negocio (podes elegir varios)',tipos);
  h+=fgrupo('calU','Calificacion ubicacion',['A','B','C','D']);
  h+=fgrupo('trans','Transito',['Alto','Medio','Bajo']);
  if(comps.length)h+=fgrupo('comp','Competencia (podes elegir varias)',comps);
  if((D.cfg.tiposProducto||[]).length)h+=fgrupo('prods','Productos que le queremos vender (podes elegir varios)',(D.cfg.tiposProducto||[]));
  h+='<div style="display:flex;gap:8px;margin-top:4px">';
  h+='<button class="btn sec" style="flex:1;margin:0" onclick="vcF=vcFvacio();renderVC();cMod()">Limpiar</button>';
...
function verEnMapaMini(id){
  ...
  setTimeout(function(){
    ...
    L.circleMarker([c.lat,c.lng],{radius:11,fillColor:(EC[etaMini]||'#22d3ee'),color:'#0b1220',weight:2,fillOpacity:.95}).addTo(miniMapaObj);
...
// Pide los datos que fabrica necesita antes de dar de alta el cliente.
// Devuelve true si quedaron cargados. Se llama en TODOS los caminos que
// convierten un prospecto en Cliente Activo.
function pedirDatosAlta(cid,despues){
  var c=D.cli.find(function(x){return x.id===cid;});if(!c)return;
  var faltan=[];
  if(!c.cuit)faltan.push('CUIT');
  if(!c.horarios)faltan.push('horarios');
  if(!faltan.length){if(despues)despues();return;}
  var h='<div style="font-size:15px;font-weight:800">'+es(c.nm)+'</div>';
  h+='<div style="font-size:12px;color:var(--red);font-weight:700;margin:6px 0 12px">Para darlo de alta como cliente falta cargar: '+faltan.join(' y ')+'.</div>';
  h+='<div style="font-size:12px;color:var(--muted);margin-bottom:12px">Son los datos que fabrica necesita en el encabezado del pedido.</div>';
  h+='<div class="fg"><label class="fl">CUIT</label><input class="fi" id="daCuit" inputmode="numeric" value="'+es(c.cuit||'')+'" placeholder="20321547460"></div>';
  h+='<div class="fg"><label class="fl">Horarios del negocio</label><input class="fi" id="daHor" value="'+es(c.horarios||'')+'" placeholder="07 A 00"></div>';
  h+='<div class="fg"><label class="fl">Condicion impositiva <span style="font-size:10px;color:var(--muted)">(opcional)</span></label><input class="fi" id="daCond" value="'+es(c.condImp||'')+'" placeholder="MONOTRIBUTO"></div>';
  h+='<div class="fg"><label class="fl">Localidad <span style="font-size:10px;color:var(--muted)">(opcional)</span></label><input class="fi" id="daCiu" value="'+es(c.ciu||'')+'"></div>';
  h+='<button class="btn" onclick="guardarDatosAlta(\''+cid+'\')" style="margin:0">Guardar y continuar</button>';
  _altaDespues=despues||null;
  oMod('Datos para dar de alta',h);
}
var _altaDespues=null;
function guardarDatosAlta(cid){
  if(soloLectura())return;
  var c=D.cli.find(function(x){return x.id===cid;});if(!c)return;
  var cuit=(document.getElementById('daCuit')||{}).value||'';
  var hor=(document.getElementById('daHor')||{}).value||'';
  if(!cuit.trim()||!hor.trim()){toast('El CUIT y los horarios son obligatorios','err');return;}
  c.cuit=cuit.trim();
  c.horarios=hor.trim();
  var cond=(document.getElementById('daCond')||{}).value;if(cond!==undefined)c.condImp=cond.trim();
  var ciu=(document.getElementById('daCiu')||{}).value;if(ciu!==undefined&&ciu.trim())c.ciu=ciu.trim();
  c._modBy=D.user?D.user.n:'?';c._modAt=new Date().toISOString();
  fsSetContacto(c);
  logEvento('edicion',c.id,c.nm,'Datos de alta cargados (CUIT y horarios)','','');
  toast('Datos guardados','ok');
  cMod();
  var f=_altaDespues;_altaDespues=null;
  if(f)f();
  refrescarVistaActual();
}
...
// Agrupa las paradas del dia por barrio, respetando el orden que ya tenian
// dentro de cada barrio. Devuelve [{barrio, gs:[...]}] en orden de recorrido.
function gruposGiraDia(plan){
  var orden=[],mapa={};
  plan.forEach(function(g){
    var c=D.cli.find(function(x){return x.id===g.cid;});
    var b=((c&&c.bar)||'').trim()||'Sin barrio';
    if(!mapa[b]){mapa[b]=[];orden.push(b);}
    mapa[b].push(g);
  });
  return orden.map(function(b){return {barrio:b,gs:mapa[b]};});
}
// Reescribe el campo 'orden' de todas las paradas del dia segun una lista de
// barrios. Es lo que hace que el mapa dibuje la ruta por zona.
function aplicarOrdenGira(fecha,grupos){
  var n=0;
  grupos.forEach(function(gr){
    gr.gs.forEach(function(g){
      if(g.orden!==n){g.orden=n;fsSetGira(g);}
      n++;
    });
  });
}
function agruparGiraPorBarrio(fecha){
  if(soloLectura())return;
  var plan=misGira().filter(function(g){return g.fecha===fecha;}).sort(function(a,b){return(a.orden||0)-(b.orden||0);});
  if(!plan.length)return;
  var grupos=gruposGiraDia(plan);
  // Los barrios con mas paradas primero: se arranca por la zona mas cargada
...
// Mueve un barrio entero al principio o al final del recorrido
function moverBarrio(fecha,barrio,dir){
  if(soloLectura())return;
  var plan=misGira().filter(function(g){return g.fecha===fecha;}).sort(function(a,b){return(a.orden||0)-(b.orden||0);});
  var grupos=gruposGiraDia(plan);
  var i=grupos.findIndex(function(gr){return gr.barrio===barrio;});
  if(i<0)return;
...
function encabezadoBarrioGira(fecha,barrio,cuantos,esPrimero,esUltimo){
  var h='<div style="display:flex;align-items:center;gap:8px;padding:12px 14px 6px">';
  h+='<div style="font-size:12px;font-weight:800;color:var(--cyan);text-transform:uppercase;letter-spacing:.4px">&#128205; '+es(barrio)+'</div>';
  h+='<div style="font-size:11px;color:var(--muted)">'+cuantos+'</div>';
  h+='<div style="flex:1;height:1px;background:var(--border)"></div>';
  if(!esPrimero)h+='<button class="lx" title="Llevar este barrio al principio" style="color:var(--cyan)" onclick="moverBarrio(\''+fecha+'\',\''+es(barrio).replace(/'/g,"")+'\',\'arriba\')">&#9650;</button>';
  if(!esUltimo)h+='<button class="lx" title="Llevar este barrio al final" style="color:var(--cyan)" onclick="moverBarrio(\''+fecha+'\',\''+es(barrio).replace(/'/g,"")+'\',\'abajo\')">&#9660;</button>';
  h+='</div>';
  return h;
}
...
function pintarMarcadores(mapa,capaVieja,lista,filtroEtapa,esAdmin,filtroTipo){
  ...
  lista.forEach(function(c){
    ...
    pop+='<div style="font-size:11px;margin-top:3px"><span style="background:'+col+';color:'+(esNeg?'#0b1220':'#fff')+';padding:2px 8px;border-radius:10px;font-weight:700">'+es(eta)+'</span></div>';
...
// ══════════════════════════════════════════════════════════════════════
// VENTAS: los pedidos que se cargaron y la plata que esta en la calle.
// Antes los pedidos solo se veian entrando cliente por cliente, y el panel de
// deudores solo aparecia si ya habia alguien debiendo: si estaba vacio no
// habia forma de llegar. Ahora es una pestania fija.
// ══════════════════════════════════════════════════════════════════════
...
function misPedidos(){
  ...
  return D.ped.filter(function(p){return mios[p.cid]||p.vend===D.user.n;});
}
function desdePeriodo(p){
  var d=new Date();
  if(p==='hoy')return today();
  if(p==='sem'){d.setDate(d.getDate()-(d.getDay()===0?6:d.getDay()-1));return fechaLocal(d);}
  if(p==='mes')return today().slice(0,7)+'-01';
  return '2000-01-01';
}
...
var gPSolapa='ped';
function setGPSolapa(x){gPSolapa=x;renderGP();}
function renderGP(){
  var top=document.getElementById('gPF'),cont=document.getElementById('gPB');
  if(!top||!cont)return;
  var deuL=deudores();
  var totDeu=deuL.reduce(function(t,d){return t+d.saldo;},0);
  var _faltaG=faltanPreguntar(), _pendG=misPendientes().length;
  top.innerHTML='<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+
    '<span class="fb'+(gPSolapa==='ronda'?' on':'')+'" onclick="setGPSolapa(\'ronda\')">Ronda'+(_faltaG?' ('+_faltaG+')':'')+'</span>'+
    '<span class="fb'+(gPSolapa==='entr'?' on':'')+'" onclick="setGPSolapa(\'entr\')">Entregas'+(_pendG?' ('+_pendG+')':'')+'</span>'+
    '<span class="fb'+(gPSolapa==='ped'?' on':'')+'" onclick="setGPSolapa(\'ped\')">Pedidos</span>'+
    '<span class="fb'+(gPSolapa==='deu'?' on':'')+'" onclick="setGPSolapa(\'deu\')">Deudores'+(deuL.length?' ('+deuL.length+')':'')+'</span>'+
    '<span class="fb'+(gPSolapa==='pagos'?' on':'')+'" onclick="setGPSolapa(\'pagos\')">Pagos</span>'+
    '</div>';
  // gVendSel: si el admin esta mirando a un vendedor puntual, se respeta
  var base=D.ped.slice();
  if(gVendSel){var ids={};cliGlobal().forEach(function(c){ids[c.id]=true;});base=base.filter(function(p){return ids[p.cid];});}
  cont.innerHTML=gPSolapa==='ronda'?htmlRondaPanel():
                 gPSolapa==='entr'?htmlEntregasPanel():
                 gPSolapa==='pagos'?htmlPagosPanel():
                 gPSolapa==='deu'?htmlDeudoresPanel(deuL,totDeu):
                 htmlPedidosPanel(base);
}
function renderVV(){
  var top=document.getElementById('vVF'),cont=document.getElementById('vVB');
  if(!top||!cont)return;
  var deuL=deudores();
  var totDeu=deuL.reduce(function(t,d){return t+d.saldo;},0);
  var _falta=faltanPreguntar(), _pend=misPendientes().length;
  top.innerHTML='<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+
    '<span class="fb'+(vVSolapa==='ronda'?' on':'')+'" onclick="setVVSolapa(\'ronda\')">Ronda'+(_falta?' ('+_falta+')':'')+'</span>'+
    '<span class="fb'+(vVSolapa==='entr'?' on':'')+'" onclick="setVVSolapa(\'entr\')">Entregas'+(_pend?' ('+_pend+')':'')+'</span>'+
    '<span class="fb'+(vVSolapa==='ped'?' on':'')+'" onclick="setVVSolapa(\'ped\')">Pedidos</span>'+
    '<span class="fb'+(vVSolapa==='deu'?' on':'')+'" onclick="setVVSolapa(\'deu\')">Deudores'+(deuL.length?' ('+deuL.length+')':'')+'</span>'+
    '<span class="fb'+(vVSolapa==='pagos'?' on':'')+'" onclick="setVVSolapa(\'pagos\')">Pagos</span>'+
    '</div>';
  cont.innerHTML=vVSolapa==='ronda'?htmlRondaPanel():
                 vVSolapa==='entr'?htmlEntregasPanel():
                 vVSolapa==='pagos'?htmlPagosPanel():
                 vVSolapa==='deu'?htmlDeudoresPanel(deuL,totDeu):
                 htmlPedidosPanel(misPedidos());
}
// ── Solapa Pedidos ───────────────────────────────────────────────────
function htmlPedidosPanel(base){
  ...
  ps.forEach(function(p){
    if(p.fecha!==diaAnt){
      diaAnt=p.fecha;
      var delDia=ps.filter(function(x){return x.fecha===p.fecha;});
...
function pagosDelPeriodo(per){
  ...
  if(D.user&&D.user.r==='vendedor'){
    ...
    ms=ms.filter(function(m){return ids[m.cid];});
  }
  return ms.sort(function(a,b){return (b.fecha||'').localeCompare(a.fecha||'');});
}
var vPagosPer='mes';
function setPagosPer(p){vPagosPer=p;if(D.user&&(D.user.r==='admin'||D.user.r==='gerente'))renderGP();else renderVV();}
function htmlPagosPanel(){
  var ms=pagosDelPeriodo(vPagosPer);
  ...
  if(!ms.length){h+='<div class="empty">Sin pagos en este periodo.</div></div>';return h;}
  var diaAnt='';
  ms.forEach(function(m){
    if(m.fecha!==diaAnt){
      diaAnt=m.fecha;
      var tDia=ms.filter(function(x){return x.fecha===m.fecha;}).reduce(function(t,x){return t+Number(x.monto||0);},0);
      h+='<div style="display:flex;align-items:center;gap:8px;margin:14px 0 8px"><div style="font-size:12px;font-weight:800;color:var(--cyan)">'+fmt(m.fecha)+'</div><div style="font-size:11px;color:var(--green);font-weight:700">'+plata(tDia)+'</div><div style="flex:1;height:1px;background:var(--border)"></div></div>';
    }
    var sal=saldoDe(m.cid);
    h+='<div onclick="verCuenta(\''+m.cid+'\')" style="display:flex;align-items:center;gap:8px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.06);cursor:pointer">';
    h+='<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700">'+es(m.cliente||'')+'</div>';
    h+='<div style="font-size:11px;color:var(--muted)">'+(m.concepto?es(m.concepto)+' · ':'')+(m.vend?es(m.vend):'')+'</div></div>';
    h+='<div style="text-align:right;flex-shrink:0"><div style="font-size:14px;font-weight:800;color:var(--green)">'+plata(m.monto)+'</div>';
    h+='<div style="font-size:10px;color:'+(sal>0?'var(--red)':'var(--muted)')+'">'+(sal>0?'debe '+plata(sal):'al dia')+'</div></div>';
    h+='<span style="color:var(--muted)">&rsaquo;</span></div>';
  });
  h+='</div>';
  return h;
}
// ── Solapa RONDA: a quien falta preguntarle el pedido esta semana ────
// Marcar "no pide esta vez" se guarda en el contacto con la fecha del ciclo,
// asi la marca se cae sola cuando arranca el ciclo siguiente.
function clientesDeLaRonda(){
  var base=misContactos(true).filter(function(c){return !c.esP&&!c.eliminado;});
  if(D.user&&(D.user.r==='admin'||D.user.r==='gerente')&&gVendSel)
    base=base.filter(function(c){return c.vend===gVendSel;});
  var ciclo=cicloActual();
  return base.map(function(c){
    var ped=D.ped.find(function(p){return p.cid===c.id&&estadoPedido(p)!=='anulado'&&(p.fecha||'')>=ciclo.desde;});
    var paso=c.sinPedido===ciclo.hasta;
    return {c:c, ped:ped||null, estado: ped?'pidio':(paso?'paso':'falta')};
  }).sort(function(a,b){
    var o={falta:0,paso:1,pidio:2};
    if(o[a.estado]!==o[b.estado])return o[a.estado]-o[b.estado];
    return (a.c.nm||'').localeCompare(b.c.nm||'');
  });
}
function faltanPreguntar(){
  return clientesDeLaRonda().filter(function(x){return x.estado==='falta';}).length;
}
// "Esta vez no pide": se marca con la fecha de entrega del ciclo
function marcarSinPedido(cid,deshacer){
  if(soloLectura())return;
  var c=D.cli.find(function(x){return x.id===cid;});if(!c)return;
  c.sinPedido=deshacer?'':cicloActual().hasta;
  c._modBy=D.user?D.user.n:'?';c._modAt=new Date().toISOString();
  fsSetContacto(c);
  if(D.user&&(D.user.r==='admin'||D.user.r==='gerente'))renderGP();else renderVV();
}
function htmlRondaPanel(){
  var lista=clientesDeLaRonda();
  var ciclo=cicloActual();
  var pidio=lista.filter(function(x){return x.estado==='pidio';}).length;
  var paso=lista.filter(function(x){return x.estado==='paso';}).length;
  var falta=lista.filter(function(x){return x.estado==='falta';}).length;
  var f=function(d){return new Date(d+'T12:00:00').toLocaleDateString('es-AR',{weekday:'short',day:'numeric',month:'short'});};
  var h='<div style="padding:12px 14px;background:var(--s1);border-bottom:1px solid var(--border)">';
  h+='<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Ciclo en curso</div>';
  h+='<div style="font-size:13px;font-weight:700;text-transform:capitalize;margin-bottom:8px">'+es(f(ciclo.desde))+' &rarr; entrega '+es(f(ciclo.hasta))+'</div>';
  h+='<div style="display:flex;gap:14px;flex-wrap:wrap">';
  h+='<div><div style="font-size:22px;font-weight:800;color:'+(falta?'var(--red)':'var(--green)')+'">'+falta+'</div><div style="font-size:10px;color:var(--muted)">FALTA PREGUNTAR</div></div>';
  h+='<div><div style="font-size:22px;font-weight:800;color:var(--green)">'+pidio+'</div><div style="font-size:10px;color:var(--muted)">YA PIDIERON</div></div>';
  h+='<div><div style="font-size:22px;font-weight:800;color:var(--muted)">'+paso+'</div><div style="font-size:10px;color:var(--muted)">NO PIDEN</div></div>';
  h+='</div></div><div style="padding:10px 14px">';
  if(!lista.length){h+='<div class="empty">Todavia no tenes clientes activos.</div></div>';return h;}
  if(!falta)h+='<div style="background:rgba(74,222,128,.10);border:1px solid rgba(74,222,128,.3);border-radius:var(--rsm);padding:10px 12px;margin-bottom:10px;font-size:13px;font-weight:700;color:var(--green)">&#10003; No falta preguntarle a nadie</div>';
  var grupoAnt='';
  lista.forEach(function(it){
    if(it.estado!==grupoAnt){
      grupoAnt=it.estado;
      var tit={falta:'FALTA PREGUNTAR',paso:'NO PIDEN ESTA VEZ',pidio:'YA PASARON PEDIDO'}[it.estado];
      var colT={falta:'var(--red)',paso:'var(--muted)',pidio:'var(--green)'}[it.estado];
      h+='<div style="display:flex;align-items:center;gap:8px;margin:14px 0 8px"><div style="font-size:12px;font-weight:800;color:'+colT+'">'+tit+'</div><div style="flex:1;height:1px;background:var(--border)"></div></div>';
    }
    var c=it.c;
    h+='<div class="cc" style="margin-bottom:8px;padding:11px">';
    h+='<div style="display:flex;align-items:center;gap:8px"><div style="flex:1;min-width:0">';
    h+='<div style="font-size:14px;font-weight:700">'+es(c.nm)+'</div>';
    h+='<div style="font-size:11px;color:var(--muted)">'+es(c.bar||c.ciu||'')+(c.uv?' · ultimo pedido '+fmt(c.uv):'')+'</div>';
    if(it.ped)h+='<div style="font-size:12px;font-weight:700;color:var(--green)">'+plata(it.ped.total)+' · '+labelEstadoPed(estadoPedido(it.ped)).toLowerCase()+'</div>';
    h+='</div></div>';
    h+='<div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">';
    if(it.estado==='pidio'){
...
// ── Solapa ENTREGAS: los pedidos tomados esperando el miercoles ──────
// Aca es donde la mercaderia realmente baja y, por lo tanto, donde nace la
...
function htmlEntregasPanel(){
  ...
  var h='<div style="padding:12px 14px;background:var(--s1);border-bottom:1px solid var(--border)">';
  h+='<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Proxima entrega</div>';
  h+='<div style="font-size:15px;font-weight:800;color:var(--cyan);text-transform:capitalize;margin-bottom:8px">'+es(new Date(prox+'T12:00:00').toLocaleDateString('es-AR',{weekday:'long',day:'numeric',month:'long'}))+'</div>';
  h+='<div style="display:flex;gap:14px;flex-wrap:wrap;align-items:center">';
  ...
  ps.forEach(function(p){
    ...
    h+='<div class="cc" style="margin-bottom:8px;border-left:4px solid #fbbf24">';
...
// El momento en que la mercaderia baja de verdad
function marcarEntrega(pid,estado){
  if(soloLectura())return;
...
function htmlDeudoresPanel(lista,tot){
  ...
  agruparPorBarrio(lista).forEach(function(g){
    ...
    g.items.forEach(function(it){h+=tarjetaDeudor(it,'tab');});
  });
  h+='</div>';
  return h;
}
// Elegir a quien cargarle una deuda vieja, sin tener que entrar a su ficha
function elegirClienteDeuda(){
  var lista=misContactos(true).filter(function(c){return !c.esP;}).sort(function(a,b){return(a.nm||'').localeCompare(b.nm||'');});
  var h='<div class="srch" style="margin:0 0 10px"><input type="text" id="dqBusq" placeholder="Buscar cliente..." oninput="filtrarClienteDeuda()" style="background:none;border:none;outline:none;color:var(--text);font-size:14px;width:100%;font-family:inherit"></div>';
  h+='<div id="dqLista" style="max-height:52vh;overflow-y:auto">'+listaClienteDeudaHTML(lista)+'</div>';
  oMod('Cargar deuda - elegir cliente',h);
}
function listaClienteDeudaHTML(lista){
  if(!lista.length)return '<div style="color:var(--muted);font-size:13px;padding:10px 0">Sin resultados.</div>';
  var h='';
  lista.slice(0,40).forEach(function(c){
    var sal=saldoDe(c.id);
    h+='<div onclick="cMod();cargarDeudaManual(\''+c.id+'\')" style="display:flex;align-items:center;gap:8px;padding:10px 4px;border-bottom:1px solid var(--border);cursor:pointer">';
    h+='<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700">'+es(c.nm)+'</div>';
    h+='<div style="font-size:11px;color:var(--muted)">'+es(c.bar||c.ciu||'')+'</div></div>';
    if(sal>0)h+='<span style="font-size:12px;font-weight:800;color:var(--red)">'+plata(sal)+'</span>';
    h+='<span style="color:var(--cyan);font-size:18px">&rsaquo;</span></div>';
  });
  return h;
}
function filtrarClienteDeuda(){
  var q=(document.getElementById('dqBusq').value||'').toLowerCase();
  var lista=misContactos(true).filter(function(c){return !c.esP&&((c.nm||'').toLowerCase().indexOf(q)>=0||(c.fan||'').toLowerCase().indexOf(q)>=0||(c.bar||'').toLowerCase().indexOf(q)>=0);}).sort(function(a,b){return(a.nm||'').localeCompare(b.nm||'');});
  document.getElementById('dqLista').innerHTML=listaClienteDeudaHTML(lista);
}
...
function renderGD(){
  ...
  var retAdm=freezersARetirar();
  if(retAdm.length)alertas.push({t:'r',msg:retAdm.length+' freezer'+(retAdm.length!==1?'s':'')+' para retirar: sin pedidos hace +'+diasRecaida()+' dias',ids:retAdm.map(function(r){return r.c.id;})});
  var deuAdm=deudores();
...
function renderGC(){
  ...
  if(gCFo.tipNeg.length)cs=cs.filter(function(c){return gCFo.tipNeg.indexOf(c.tipo)>=0;});
  if(gCFo.comp.length)cs=cs.filter(function(c){return gCFo.comp.some(function(cp){return c.comp&&c.comp.includes(cp);});});
  if(gCFo.frez.length)cs=cs.filter(function(c){return gCFo.frez.indexOf(c.cFr)>=0;});
  if(gCFo.eta.length)cs=cs.filter(function(c){return gCFo.eta.indexOf(c.etapaEmbudo)>=0;});
  if(gCFo.calU.length)cs=cs.filter(function(c){return gCFo.calU.indexOf(c.calU)>=0;});
  if(gCFo.fuente.length)cs=cs.filter(function(c){return gCFo.fuente.indexOf(c.fuente)>=0;});
  ...
  else if(gCF2==='Deudores')cs=cs.filter(function(c){return saldoDe(c.id)>0;});
  ...
  if(vistaModo==='lista'){
    cs.forEach(function(c){
      ...
      if(saldoDe(c.id)>0)h+='<span style="background:var(--red);color:#fff;padding:2px 6px;border-radius:5px;font-size:9px;font-weight:900;flex-shrink:0">DEU</span>';
      h+='</div>';
    });
  ...
  cs.forEach(function(c){
    ...
    if(saldoDe(c.id)>0)h+='<span style="background:var(--red);color:#fff;padding:4px 10px;border-radius:6px;font-size:11px;font-weight:900;letter-spacing:.5px">⚠ DEBE '+plata(saldoDe(c.id))+'</span>';
    h+='</div></div>';
...
function proximaEntrega(desde){
  var d=desde?new Date(desde+'T12:00:00'):new Date();
  var obj=diaEntrega();
  var delta=(obj-d.getDay()+7)%7;
  d.setDate(d.getDate()+delta);
  return fechaLocal(d);
}
// La ventana de toma de pedidos del ciclo actual: desde el dia siguiente a la
// ultima entrega, hasta la proxima entrega.
function cicloActual(){
  var hasta=proximaEntrega();
  var d=new Date(hasta+'T12:00:00');
  d.setDate(d.getDate()-6);
  return {desde:fechaLocal(d), hasta:hasta};
}
function estadoPedido(p){ return p.estado||'entregado'; }
function pedidoEntregado(p){ return estadoPedido(p)==='entregado'; }
function pedidosPendientes(){
  return D.ped.filter(function(p){return estadoPedido(p)==='tomado';})
    .sort(function(a,b){return (a.fecha||'').localeCompare(b.fecha||'');});
}
// Los pedidos pendientes del usuario actual
function misPendientes(){
  var ps=pedidosPendientes();
  if(!D.user||D.user.r==='admin'||D.user.r==='gerente'){
    if(!gVendSel)return ps;
    var ids={};cliGlobal().forEach(function(c){ids[c.id]=true;});
    return ps.filter(function(p){return ids[p.cid];});
  }
  var mios={};D.cli.forEach(function(c){if(c.vend===D.user.n)mios[c.id]=true;});
  return ps.filter(function(p){return mios[p.cid]||p.vend===D.user.n;});
}
function labelEstadoPed(e){ return {tomado:'TOMADO',entregado:'ENTREGADO',anulado:'NO ENTREGADO'}[e]||'ENTREGADO'; }
function colorEstadoPed(e){ return {tomado:'#fbbf24',entregado:'#4ade80',anulado:'#f87171'}[e]||'#4ade80'; }
...
function abrirPedido(cid){
  ...
  var sal=saldoDe(cid);
  if(sal>0){
    var prom=compromisoDe(cid);
    var msg='"'+c.nm+'" DEBE '+plata(sal)+'.';
    if(prom)msg+='\nSe comprometio a pagar el '+fmt(prom)+(prom<today()?' (ya vencio)':'')+'.';
...
function textoFabrica(){
  ...
  if(esPrimerPedido(c.id)){
    ...
    // Encabezado corto: negocio en NEGRITA (los asteriscos los interpreta
    // WhatsApp), ciudad y horarios.
    L.push('*'+(c.nm||'')+'*');
    if(c.dir)L.push(c.dir);
    L.push('Localidad: '+(c.ciu||''));
    L.push('Horarios: '+(c.horarios||''));
  }
...
// Fecha en que el cliente se comprometio a pagar el saldo, si dio una.
function compromisoDe(cid){
  var c=D.cli.find(function(x){return x.id===cid;});
  return (c&&c.promPago)||'';
}
// Guarda (o borra) el compromiso de pago del cliente
function setCompromiso(cid,fecha){
  var c=D.cli.find(function(x){return x.id===cid;});if(!c)return;
  var ant=c.promPago||'';
  c.promPago=fecha||'';
  if(ant!==c.promPago){
...
function deudores(){
  ...
  (D.deudas||[]).forEach(function(m){
    ...
    var prom=compromisoDe(m.cid);
    out.push({c:c,saldo:sal,desde:desde,dias:desde?dias(desde):null,
      prom:prom,
      promVenc:!!(prom&&prom<today()),      // prometio pagar y no pago
      promHoy:!!(prom&&prom===today())});
  });
  // Primero el que rompio la promesa, despues el que la tiene para hoy, y
  // dentro de cada grupo el que mas debe.
  out.sort(function(a,b){
    var pa=a.promVenc?0:(a.promHoy?1:2), pb=b.promVenc?0:(b.promHoy?1:2);
...
// Antes esto mantenia una marca "deu" si/no en el contacto. Esa marca se saco:
// el saldo se calcula siempre de los movimientos. Lo unico que queda es soltar
// el compromiso de pago cuando el cliente termina de pagar.
function sincronizarMarcaDeudor(cid){
  var c=D.cli.find(function(x){return x.id===cid;});if(!c)return;
  if(saldoDe(cid)<=0&&c.promPago){
    c.promPago='';
    c._modBy=D.user?D.user.n:'?';c._modAt=new Date().toISOString();
    fsSetContacto(c);
  }
...
function registrarPago(cid,volverA){
  ...
  // Compromiso: solo tiene sentido si queda saldo. El campo esta siempre y se
  // ignora si paga todo, para no hacer dos pantallas distintas.
  var sem=new Date();sem.setDate(sem.getDate()+7);
  h+='<div class="fg"><label class="fl">Si queda debiendo, cuando viene por el resto <span style="font-size:10px;color:var(--muted)">(opcional)</span></label>';
  h+='<input class="fi" type="date" id="pgProm" min="'+today()+'" value="'+(compromisoDe(cid)||fechaLocal(sem))+'"></div>';
...
function guardarPago(cid,volverA){
  ...
  var promEl=document.getElementById('pgProm');
  nuevoMovimiento(cid,'pago',monto,nota?nota.value.trim():'','');
  var resto=saldoDe(cid);
  // El compromiso se guarda solo si quedo saldo (si pago todo, sincronizarMarcaDeudor ya lo limpio)
  if(resto>0&&promEl&&promEl.value){
    setCompromiso(cid,promEl.value);
...
// Tarjeta de un deudor. 'panel' es el modal de HOY (al cobrar vuelve al modal);
// 'tab' es la pestania Ventas (al cobrar se refresca la pestania).
function tarjetaDeudor(it,ctx){
  var c=it.c;
  var vuelta=ctx==='tab'?'tab':'panel';
  var col=it.dias===null?'var(--muted)':it.dias>30?'var(--red)':it.dias>15?'var(--orange)':'var(--yellow)';
  if(it.promVenc)col='var(--red)';
  var h='<div class="cc" style="margin-bottom:8px;border-left:4px solid '+col+'">';
  if(it.promVenc)h+='<div style="font-size:11px;font-weight:800;color:var(--red);margin-bottom:4px">&#9888; PROMETIO PAGAR EL '+fmt(it.prom)+' Y NO PAGO</div>';
  else if(it.promHoy)h+='<div style="font-size:11px;font-weight:800;color:var(--yellow);margin-bottom:4px">&#128197; HOY VIENE POR EL SALDO</div>';
  h+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><div style="flex:1;min-width:0">';
  h+='<div style="font-size:14px;font-weight:700">'+es(c.nm)+'</div>';
  if(c.dir)h+='<div style="font-size:11px;color:var(--muted)">&#128205; '+es(c.dir)+'</div>';
  h+='<div style="font-size:11px;color:'+col+'">'+(it.dias===null?'sin fecha':'hace '+it.dias+' dias')+(it.prom&&!it.promVenc&&!it.promHoy?' · paga el '+fmt(it.prom):'')+'</div>';
  h+='</div><div style="font-size:17px;font-weight:900;color:var(--red);flex-shrink:0">'+plata(it.saldo)+'</div></div>';
  h+='<div style="display:flex;gap:6px;flex-wrap:wrap">';
  h+='<button class="sm g" onclick="registrarPago(\''+c.id+'\',\''+vuelta+'\')">Cobrar</button>';
...
function verDeudores(){
  ...
  if(!lista.length){h='<div class="empty">Nadie debe nada. Muy bien!</div>';}
    ...
    agruparPorBarrio(lista).forEach(function(g){
      var totB=g.items.reduce(function(t,d){return t+d.saldo;},0);
      h+='<div style="display:flex;align-items:center;gap:8px;margin:14px 0 8px"><div style="font-size:12px;font-weight:800;color:var(--cyan);text-transform:uppercase;letter-spacing:.4px">&#128205; '+es(g.barrio)+'</div><div style="font-size:11px;color:var(--red);font-weight:700">'+plata(totB)+'</div><div style="flex:1;height:1px;background:var(--border)"></div></div>';
      g.items.forEach(function(it){h+=tarjetaDeudor(it,'panel');});
    });
  }
...
function renderGE(){
  ...
  document.getElementById('gEF').innerHTML=all.map(function(e){return '<span class="fb'+(gEF2===e?' on':'')+'" onclick="sGEF(\''+es(e)+'\')">'+es(e)+'</span>';}).join('')+'<span style="margin-left:auto">'+botonesVistaHTML()+'</span>';
  var base=cliGlobal();
...
function avanzarComodato(id,nuevoEstado){
  ...
  if(nuevoEstado==='por_entregar'){
    ...
    // La carga inicial NO se da por hecha aca: es un pedido como cualquier
    // otro, que se toma en su dia y se entrega el miercoles.
    var c=D.cli.find(function(x){return x.id===co.cid;});
    var _conv=false;
    if(c){
      if(c.esP)_conv=true;
...
function saveCo(){
  ...
  D.com.push(nCom);
  // Si se entrega de una, el cliente pasa a activo y la carga inicial cuenta
  // como su primer pedido (baja junto con el equipo).
  if(estado==='activo'&&c){
    c.esP=false;c.etapaEmbudo='Cliente Activo';
    fsSetContacto(c);
  }
...
function renderGCfg(){
  ...
  if(enRec.length){
    h+='<div style="font-size:11px;color:var(--yellow);margin-top:10px;line-height:1.5"><b>'+enRec.length+' cliente'+(enRec.length===1?'':'s')+' caeria'+(enRec.length===1?'':'n')+' con este plazo:</b><br>'+enRec.map(function(c){var fz=fechaFreezerPuesto(c.id);return es(c.nm)+' ('+(c.uv?'ultimo pedido '+fmt(c.uv):(fz?'sin pedidos, freezer puesto el '+fmt(fz):'sin pedidos, ingreso '+fmt(c.ing)))+')';}).join('<br>')+'</div>';
  } else {
...
function cMod(){
  if(volverAlWizard){volverAlWizard=false;var _w=document.getElementById('sWiz');if(_w)_w.classList.add('on');}document.getElementById('modal').classList.remove('on');}
...
function wFin(){
  ...
  if(c){
    ...
    // El pedido ya actualizo uv y la deuda: aca solo queda el vinculo
...
// Lo que se ve cuando el vendedor marca que SI vendio: o el boton para tomar
// el pedido, o el resumen del pedido que ya cargo en esta misma visita.
function pedidoEnVisitaHTML(){
  var p=W.data.pid?D.ped.find(function(x){return x.id===W.data.pid;}):null;
  if(p){
    var debe=p.cobrado!==undefined?(Number(p.total||0)-Number(p.cobrado||0)):0;
    var h='<div class="card" style="border:1px solid var(--green)"><div class="ct" style="color:var(--green)">PEDIDO CARGADO</div>';
    h+='<div style="font-size:20px;font-weight:900;color:var(--green)">'+plata(p.total)+'</div>';
  ...
  return '<div class="card"><div style="font-size:12px;color:var(--muted);margin-bottom:8px">Carga el pedido con los productos: de ahi salen el total, el texto para fabrica y la deuda si no lo cobraste entero.</div>'+
    '<button class="btn" onclick="tomarPedidoEnVisita()" style="margin:0">Tomar el pedido</button></div>';
}
// Abre el modulo de pedidos desde el wizard. Al guardar, guardarPedido detecta
// que venimos de una visita y vuelve solo a este paso.
var volverAlWizard=false;
function tomarPedidoEnVisita(){
  if(!W.cid)return;
  volverAlWizard=true;
  abrirPedido(W.cid);
}
// Refresca el bloque del pedido cuando se vuelve de cargarlo
function refrescarPedidoEnVisita(){
  var box=document.getElementById('nvSi');
  if(box)box.innerHTML=pedidoEnVisitaHTML();
}
function tgV(si){
  W.data.vendio=si;
...
function renderVC(){
  ...
  if(vcF.vis.length)cs=cs.filter(function(c){return (vcF.vis.indexOf('v')>=0&&!!c.ul)||(vcF.vis.indexOf('sv')>=0&&!c.ul);});
  if(vcF.frez.length)cs=cs.filter(function(c){var sin=(c.cFr==='Sin freezer'||!c.cFr),pro=c.cFr==='Propio',comp=c.cFr==='Competencia';return (vcF.frez.indexOf('sin')>=0&&sin)||(vcF.frez.indexOf('pro')>=0&&pro)||(vcF.frez.indexOf('comp')>=0&&comp);});
  if(vcF.comp.length)cs=cs.filter(function(c){return vcF.comp.some(function(cp){return c.comp&&c.comp.includes(cp);});});
  if(vcF.calU.length)cs=cs.filter(function(c){return vcF.calU.indexOf(c.calU)>=0;});
  if(vcF.trans.length)cs=cs.filter(function(c){return vcF.trans.indexOf(c.trans)>=0;});
  ...
  cs.sort(function(a,b){return(a.nm||'').localeCompare(b.nm||'');});
  var chips='';
  function chipF(k,txt){return '<span class="ch on" onclick="qFiltro(\''+k+'\')" style="font-size:11px;padding:5px 10px">'+txt+' &#215;</span>';}
  if(vcF.tipo_ctx.length)chips+=chipF('tipo_ctx',vcF.tipo_ctx.map(function(x){return x==='prospecto'?'Prospecto':'Cliente';}).join(', '));
  if(vcF.bar.length)chips+=chipF('bar',es(vcF.bar.join(', ')));
  if(vcF.tipOneg.length)chips+=chipF('tipOneg',es(vcF.tipOneg.join(', ')));
  if(vcF.vis.length)chips+=chipF('vis',vcF.vis.map(function(x){return x==='v'?'Visitados':'Sin visitar';}).join(', '));
  if(vcF.frez.length)chips+=chipF('frez','Freezer: '+vcF.frez.map(function(x){return x==='sin'?'Sin':x==='pro'?'Propio':'Competencia';}).join(', '));
  if(vcF.comp.length)chips+=chipF('comp','Comp: '+es(vcF.comp.join(', ')));
